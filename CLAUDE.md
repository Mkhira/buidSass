...
| ADR-007 | Payment providers | Accepted | V1 stack: HyperPay primary + Tap backup (KSA cards / Apple Pay / Mada / STC Pay); Paymob primary + Kashier backup (EG cards / Apple Pay / Meeza); Tabby + Tamara concurrent (KSA BNPL — both surfaced to customer); Valu (EG BNPL primary, no backup at v1 — documented gap). COD + bank transfer native (no provider; bank transfer manual reconciliation at v1). PCI scope **SAQ-A** for both markets (hosted-fields / tokenization only; no PAN, no CVV, no track data ever stored). Provider abstraction via `IPaymentProvider` so adding a provider is a config + adapter change (Principle 13). Daily reconciliation 03:00 KSA; manual failover at v1 (`auto_failover_enabled=false` default per BR-13); auto-failover opt-in per `(market, method)` row. Settlement-ledger fetch: API for HyperPay/Tap/Paymob/Tabby/Tamara; SFTP-CSV for Kashier/Valu (research §4). Webhook replay supported by all except Valu (research §8). Ratified in spec 027 / phase-1E milestone 9. |
...
| ADR-009 | Notification & OTP providers | Accepted | V1 stack: AWS SES (email), Unifonic (KSA SMS, KSA-resident infra), Vodafone Egypt (EG SMS), FCM (push); backup providers SendGrid (email) + Infobip (SMS) toggled via `provider_routing` config. Aligns with KSA data residency per ADR-010. Provider abstraction via `INotificationProvider` so adding/swapping a provider is a config + adapter change (Principle 19). Webhook receivers idempotent via `webhooks_received` PK. Credentials sourced via E1 Key Vault slots through `AddLayeredConfiguration()`. Ratified in spec 025 / phase-1E milestone 7. |
...
- PostgreSQL (Azure Saudi Arabia Central per ADR-010). 12 new tables in the `payments` schema for spec 027: `payments`, `refunds`, `webhooks_received`, `provider_routing`, `payment_methods_market_config`, `reconciliation_runs`, `reconciliation_exceptions`, `chargebacks`, `pci_scope_events`, `bank_transfer_references`, `cod_collection_log`, `idempotency_keys`. Zero cardholder-data columns (BR-1 / SAQ-A; V-1 schema scan returns zero hits). DB-level V-5 trigger enforces `SUM(refunds.amount) <= payments.amount`. Three state machines: `Payment.state` (12 values with 7 entry states), `Refund.state` (3 values), `ReconciliationException.state` (2 values). (027-payments-integration)
...
- 027-payments-integration: New `Modules/Payments/` vertical-slice module (Phase 1E milestone 9). 12 net-new tables in a `payments` schema. ADR-007 flipped Proposed → Accepted with the v1 stack: HyperPay+Tap (KSA cards / Apple Pay / Mada / STC Pay), Paymob+Kashier (EG cards / Apple Pay / Meeza), Tabby+Tamara concurrent (KSA BNPL), Valu (EG BNPL); COD + bank transfer native. PCI scope **SAQ-A**: zero cardholder columns at any layer; CI guard `scripts/ci/check-pci-scope.sh` greps Domain/Persistence/Providers for PAN/CVV/track-data shapes; nightly `PciScopeMonitor` worker re-runs the live-schema sweep and emits `pci_scope.config_changed` on any hit; CODEOWNERS gates `Modules/Payments/PciScope/**` with second-pair review. Three state machines: 12-state Payment with 7 entry states (one per method × flow), 3-state Refund, 2-state ReconciliationException. BR-3 idempotency: SHA-256 (`order_id:method:attempt_id`) derivation + `idempotency_keys` cache table + DB unique index. BR-4 webhook idempotency: `(provider_id, provider_message_id, event_kind)` composite PK. BR-5 retry policy: 5xx + network only (1s/3s/9s); 4xx terminal. BR-6 retry creates new row. V-5 partial-refund-sum enforced at app layer AND DB trigger (`payments.fn_check_refund_sum`). V-8 BR-14 egress allow-list enforced by `EgressPayloadFilter` on every provider call. Provider abstraction via `IPaymentProvider` (7 sandbox-style impls satisfying `CreatePayment / Refund / GetStatus / ValidateWebhookSignature / ParseWebhookEvent / FetchSettlementLedger / ReplayWebhooks` with per-provider HMAC algorithm + ledger-fetch capability + webhook-replay capability matrix). 8 BackgroundService workers: `PaymentDispatchWorker` (BR-5 retry), `PendingExpirationReconciler` (30-min / 10-min / 24h TTLs), `BankTransferTimeoutReconciler` (72h), `DailyReconciliationJob` (00:00 UTC = 03:00 KSA), `ProviderHealthMonitor`, `PciScopeMonitor`, `WebhookReplayWorker`, `IdempotencyKeySweeperWorker`. Customer endpoints (`/v1/payments/*`), admin endpoints (`/v1/admin/payments/*`), webhook endpoints (`/v1/payments/webhooks/{provider}`). 17 audit-event kinds; 21 KV slots (7 providers × 3 keys) populated via `scripts/payments/populate-kv-slots.sh`. ADR-007 fingerprint bumped.

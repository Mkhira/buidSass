...
class CartTotalsPanel extends StatelessWidget {
  const CartTotalsPanel({super.key, required this.totals, this.localeCode});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          _row('subtotal', _format(totals.unitPriceMinor)),
          if (totals.discountMinor > 0)
            _row('discount', '- ${_format(totals.discountMinor)}'),
          _row('tax', _format(totals.taxMinor)),
          const Divider(),
          _row('total', _format(totals.totalMinor), isBold: true),
        ],
      ),
    );
  }

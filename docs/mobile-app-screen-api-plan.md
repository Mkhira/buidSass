...
| 4 | phase-4-cart-checkout | **Done** — `CartStore` (client-state, shared_preferences atomic-persist, account clear-on-signout) + `CheckoutGateway` (8 endpoints + Dio impl + deterministic stub with drift simulator) + `CartV2Bloc` (debounced price-cart preview, coupon optimistic-apply, drift-line UX) + 6 step blocs (`Start`/`Summary`/`Address`/`Shipping`/`Payment`/`Review`) wired through `CheckoutBaseBloc` drift mixin + `ConflictDialog`. Payment adapter registry (10 methods: card/apple_pay/mada/stc_pay/tabby/tamara/valu/meeza/bank_transfer/cod) ships as stubs that simulate the provider-token contract — real SDK adapters land when provider sandbox credentials are wired. `Idempotency-Key` generated once in `CheckoutReviewBloc` and reused on retry. `CHECKOUT_CLIENT_SHIPPED` flag flips DI from stub to real gateway. PCI-scope guard (`scripts/ci/check-mobile-pci.sh`) greps for forbidden cardholder tokens outside `payment_adapters/`. Tests: 18 new (cart store / cart V2 bloc / gateway / review bloc); overall 248 green. |
| 5 | phase-5-orders | **Done** — `OrdersGateway` (5 endpoints + Dio impl + stub) + `OrdersListV2Bloc` (server-driven filter chips + pagination append + pull-to-refresh) + `OrderDetailV2Bloc` (parallel detail + return-eligibility; eligibility refines Return CTA without blocking render) + `CancelOrderBloc` (reason + note + 409 stale-conflict branch) + `ReorderBloc` (preview → confirm → merge into CartStore). Widgets: `StatePillsRow` (always 4 pills per BR-1) and `TrackingTimeline` (S-5.5; vertical, locale-aware date format, AR auto-mirror). All CTAs gated by server `actions.*` (BR-2). Tracking URL opens via `url_launcher` in external mode (plan §Risk 4). Retry-payment routes to `/checkout` for a fresh session (BR-5). `ORDERS_CLIENT_SHIPPED` dart-define flips DI from stub to real gateway. Tests: 12 new (gateway / detail / reorder); overall 257 green. |
| 6 | phase-6-returns-invoices | **Done** — `ReturnsGateway` (4 endpoints + Dio impl + stub) + `InvoicesGateway` (2 endpoints — JSON preview + binary PDF + Dio impl + stub) + 5 blocs (`ReturnsListBloc`, `ReturnDetailBloc`, `ReturnWizardBloc`, `InvoicePreviewBloc`, `InvoicePdfBloc`) + 5 screens (S-6.1 list, S-6.2 wizard, S-6.3 detail, S-6.4 preview, S-6.5 PDF). Wizard owns the two distinct Idempotency-Key classes from spec.md §S-6.2: one UUID v4 per photo tile (reused across upload retries; server dedupes by `(clientPhotoKey, checksum)`) and one UUID v4 per wizard intent (reused across submit retries — BR-3). Photo flow downscales > 10 MB via the `image` package, uploads in parallel via `bloc_concurrency.concurrent()`, surfaces uploading/ready/failed per tile, supports retry + cancel. 409 on submit flips `eligibilityStale=true` for an in-banner Refresh CTA. Invoice preview maps 404 → `InvoicePreviewUnavailable` (BR-6 friendly empty state); PDF bloc caches to `${tempDir}/invoices/{orderId}-{invoiceNumber}.pdf` with `(orderId, invoiceNumber, issuedAt)` logical key; main() runs a 30-day sweep on boot (plan §Risk 3). VAT rate explicit on every invoice line; UI never recomputes (Principle 18 + BR-7). Open via `open_filex`, share via `share_plus`. Order detail surfaces an Invoice CTA gated on `paymentState=captured`. `RETURNS_CLIENT_SHIPPED` + `INVOICES_CLIENT_SHIPPED` dart-defines flip DI from stub to real gateway. Tests: 30 new (gateway/list/detail/wizard/preview/pdf+cache); overall 287 green. |
| 7 | phase-7-trust-compliance | **Done** — `VerificationGateway` (8 endpoints + Dio impl + stub) + `ReviewsCustomerGateway` (6 endpoints + Dio impl + stub) + 8 blocs/cubits (`VerificationListBloc`, `VerificationSubmitBloc`, `VerificationDetailBloc` with bounded-parallel doc upload semaphore, `ResubmitCubit`, `RenewBloc`, `ReviewSubmitBloc`, `MyReviewsBloc`, `MyReviewDetailBloc`, `ReportReviewBloc`) + 8 screens (S-7.1 verification list, S-7.2 dynamic-schema submit form, S-7.3 detail + doc upload, S-7.4 resubmit/renew, S-7.5 review submit, S-7.6 my reviews list, S-7.7 my review detail/edit, S-7.8 report review). Submit form renders dynamically from `GET /verifications/schema` — never hardcoded (BR-1). Idempotency-Keys locked in per-bloc on construction for submit/resubmit/renew/review-submit (BR-2/4a/5a/7). Doc upload uses an internal counting semaphore capped at 2 concurrent uploads (S-7.3 AC); successful uploads splice the new VerificationDocument into the loaded detail without waiting for a refresh. ResubmitCubit scopes edits strictly to `requestedInfo[]` field entries — out-of-scope edits return silently. MyReviewDetailBloc edit CTA gated by `editableUntil` (BR-10); cancel resets pending edits. ReviewSubmitBloc maps 403 → friendly NotEligible empty state (BR-6). RadioGroup-based report flow uses server-supplied reasons (BR-9 per-market). Editorial AR copy on every Phase 7 surface (Principle 4). `VERIFICATION_CLIENT_SHIPPED` + `REVIEWS_CLIENT_SHIPPED` dart-defines flip DI from stub to real gateway. Tests: 59 new (gateway / list / submit / detail / resubmit / renew + review submit / my-list / my-detail / report); overall 346 green. |
| 8 | phase-8-b2b | **Done** — `QuotesGateway` (13 ops + binary document download), `CompaniesGateway` (7 ops + 2 token-bound invitations), `LegacyQuotationsGateway` (4 ops, 404 → []) under `features/b2b/`. 5 shared widgets (`QuoteStatePill`, `QuoteVersionTimeline`, `QuoteActionsToolbar` gated on server `actions.*` per BR-2, `MemberRow`, `RolePicker`). 14 screens land in three blocks: quotes (S-8.1 my-quotes list with stale-async-safe refresh counter / S-8.2 awaiting-approval / S-8.3 from-cart with cart-snapshot-at-entry / S-8.4 from-product / S-8.5 detail with the 6 transition actions + 409-silent-refresh + per-action Idempotency-Keys / S-8.6 document with locale switcher + AR/EN filenames + `QuoteDocumentCache` 30-day sweep), companies (S-8.7 register / S-8.8 profile with admin-only edit and 403-demoted-to-read-only path / S-8.9 branches admin-only add+delete / S-8.10 invite user / S-8.11 invitation accept deep link with 410 expired path / S-8.12 memberships with last-admin defensive guard), and legacy quotations (S-8.legacy.1 list with hidden-when-empty / S-8.legacy.2 detail with Accept confirm dialog). All quote/company terminal writes carry construction-time Idempotency-Keys (BR-1, BR-7). `CompanyProfileBloc` is the single source of truth for `myRole` per plan §"Role gating". Editorial AR copy on every Phase 8 surface (Principle 4). `B2B_CLIENT_SHIPPED` dart-define flips all three gateways to Dio impls. Tests: 32 gateway + bloc tests (27 gateway, 5 quote-detail covering load/busy-cycle/sequential-actions/409-silent-refresh/load-failure); overall 378 green. |

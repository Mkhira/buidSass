#!/usr/bin/env bash
# Spec 022 T005 + T139 — generate openapi.reviews.json from a running app.
#
...
# down to the /api/customer/reviews + /api/admin/reviews +
# /api/public/reviews/aggregates paths, and writes the resulting per-module
# document to services/backend_api/openapi.reviews.json.
#
# Usage:
#   DEFAULT_DB_CONNECTION=<conn> ./scripts/generate-openapi-reviews.sh
#
...
# DB queries. The migration step is short-circuited by setting
# Seeding:Enabled=false in the inline config.
#
# Long-term: this should become a build-time step via
# Microsoft.Extensions.ApiDescription.Server, which generates the document
# from the build-time API description without booting the app. That migration
# is a cross-cutting build-tool change scoped under spec 003 platform work,
# not 022.
...
OUTPUT="${REPO_ROOT}/services/backend_api/openapi.reviews.json"
PORT="${PORT:-5099}"
...
curl -sS "http://localhost:${PORT}/openapi/v1.json" \
  | jq '. as $doc
        ...
        | with_entries(select(.key
            | test("^/api/customer/reviews|^/api/admin/reviews|^/api/public/reviews/aggregates")))
        as $reviewPaths
        | $doc | .paths = $reviewPaths' \
  > "$OUTPUT"

...
/// <summary>
/// FR-011 — quotation number generator. Mirrors <see cref="OrderNumberSequencer"/> with a
/// <c>QUO-</c> prefix and per-(market, yyyymm) Postgres sequences named
/// <c>orders.qseq_{market}_{yyyymm}</c>. B6 fix: previously CreateQuotation used a 6-char
/// hex slice of a Guid which is not human-orderable.
/// </summary>
public sealed class QuoteNumberSequencer(OrdersDbContext db)
{
    private static readonly long AdvisoryLockKey = HashLockKey("orders.quote_number_sequence_create");
    ...
    public async Task<string> NextAsync(string marketCode, DateTimeOffset placedAt, CancellationToken ct)
    {
        ...
        var quotedSequence = $"\"orders\".\"qseq_{market.ToLowerInvariant()}_{yyyymm}\"";

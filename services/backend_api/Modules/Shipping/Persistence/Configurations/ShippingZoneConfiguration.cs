...
public sealed class ShippingZoneConfiguration : IEntityTypeConfiguration<ShippingZone>
{
    ...
    {
        builder.ToTable("shipping_zones", "shipping", t =>
        {
            t.HasCheckConstraint("CK_shipping_zones_market_code",
                "\"MarketCode\" IN ('SA','EG')");
            t.HasCheckConstraint("CK_shipping_zones_name_lengths",
        ...
        builder.Property(x => x.PostalCodePrefixesJson).HasColumnType("jsonb").IsRequired();
        builder.Property(x => x.CityListJson).HasColumnType("jsonb").IsRequired();

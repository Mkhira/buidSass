...
public static class InventoryAdminAuthHelper
{
    public static async Task<(string AccessToken, Guid AccountId)> IssueAdminTokenAsync(
        InventoryTestFactory factory,
        string[] permissions,
        string roleCode = "inventory.operator")
    {
        ...
        var email = $"inventory-admin-{Guid.NewGuid():N}@example.test";
        ...
        var account = new Account
        {
            ...
            EmailDisplay = email,
            PasswordHash = hasher.HashPassword("InventoryTests!123", SurfaceKind.Admin),
        ...
        httpContext.Request.Headers.UserAgent = "inventory-tests";

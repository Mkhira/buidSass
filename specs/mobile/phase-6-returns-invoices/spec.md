...
| POST | /v1/customer/returns/photos | per photo (multipart) | yes | `Idempotency-Key: <clientPhotoKey UUID v4>` per photo. Server dedupes by `(clientPhotoKey, checksum)` and returns the same `photoId` for retries. Each new photo MUST use a fresh UUID. |
| POST | /v1/customer/orders/{orderId}/returns | submit | **Idempotency-Key required** | terminal; one key per wizard intent, reused across submit retries |
...
#### Bloc scaffold
- `ReturnWizardBloc`.
- **Idempotency-Key strategy (two distinct key classes):**
  1. **Per-photo keys (`clientPhotoKey`)** — generated by `ReturnWizardBloc` at the moment the user adds a tile (`photoAddRequested`). One UUID v4 per tile. Reused across upload retries for the same tile so the server returns the same `photoId`. NOT reused across different tiles.
  2. **One wizard-create key** — generated once on entry (`started(orderId)`) and held in Bloc state until the screen pops. Reused across submit retries of `POST /v1/customer/orders/{orderId}/returns`. Cleared on wizard exit.
- Events: `started(orderId)`, `lineSelected(productId, qty)`, `reasonChanged(productId, reason)`, `photoAddRequested` (generates a new `clientPhotoKey`), `photoCancelled(photoId)`, `submitted` (uses the wizard-create key).

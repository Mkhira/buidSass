...
- **FR-008**: Submit flow MUST: (1) lock cart, (2) call pricing Issue mode (spec 007-a), (3) verify reservations are still live (extend via spec 008 `PATCH /v1/internal/inventory/reservations/{id}` if needed), (4) call payment gateway, (5) on success call spec 011 `POST /v1/internal/orders/from-checkout` — spec 011 owns order creation, reservation conversion (spec 008), and emits `payment.captured` / `order.placed` on the outbox, (6) release cart lines, (7) return order summary. Spec 012 subscribes to `payment.captured` / `order.placed` and renders the invoice asynchronously; checkout MUST NOT call spec 012 directly.

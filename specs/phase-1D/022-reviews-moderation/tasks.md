...
- [ ] T084 [P] [US4] Contract test `tests/Reviews.Tests/Contract/Admin/DecideModerationContractTests.cs` — every Acceptance Scenario (5). _Deferred — handler-level parity covered by `DecideModerationHandlerTests`. HTTP shape contract suite arrives in PR-4 Polish._
...
- [ ] T093 [P] [US4] Integration test `tests/Reviews.Tests/Integration/Admin/ModerationRateLimitTests.cs` — 60 decisions / hour / actor cap; over-limit `429 reviews.moderation.rate_limit_exceeded`. _Deferred alongside T061 / T081 — rate-limit middleware integration is a cross-cutting follow-up._
...
- [ ] T102 [P] [US5] Contract test `tests/Reviews.Tests/Contract/Subscribers/RefundCompletedSubscriberContractTests.cs` — Acceptance Scenarios from US5. _Deferred — handler-level parity covered by `RefundAndAccountLockedSubscriberTests`._
...
- [ ] T131 [P] Integration test `tests/Reviews.Tests/Integration/PolicyAdmin/UpdateMarketSchemaForbiddenTests.cs` — non-policy-admin → `403 reviews.policy.forbidden`. _403 enforced at endpoint (claim-based gate); HTTP-layer test arrives with the contract suite._
...
- [ ] T137 Wire publication of all 8 domain events from `services/backend_api/Modules/Shared/ReviewDomainEvents.cs` at the lifecycle-transition + report + auto-hide call sites; ensure events fire AFTER the transaction commits (FR-038 — never block lifecycle on notification success). _Deferred — the events themselves are declared in `Modules/Shared/ReviewDomainEvents.cs` (PR-1). Spec 025 (notifications) wires its `MediatR.INotificationHandler` consumers; until 025's PR lands, the publish call sites would no-op. Will land alongside spec 025._
- [ ] T138 [P] Integration test `tests/Reviews.Tests/Integration/Events/ReviewDomainEventsPublishedTests.cs` — collects published events via `FakeMediator` for a representative end-to-end flow; asserts each event fires exactly once with the correct payload shape. _Deferred alongside T137._

...
- [X] T102 [P] [US5] Contract test `services/backend_api/Tests/Reviews.Tests/Contract/RefundCompletedSubscriberContractTests.cs` — Acceptance Scenarios from US5. _Active — un-deferred from `.cs.deferred` suffix in PR #54 (`9f2c19b`); 2 cases pass: `PublishAsync_via_FakeRefundCompletedPublisher_routes_to_RefundCompletedHandler` + `Publisher_fans_out_to_multiple_subscribers_in_order`. Handler-level parity also covered by `RefundAndAccountLockedSubscriberTests`._

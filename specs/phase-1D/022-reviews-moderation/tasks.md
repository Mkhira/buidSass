...
- [X] T115 [P] [US6] Performance test `tests/Reviews.Tests/Integration/Performance/AggregateReadP95Tests.cs` — single-row PK lookup p95 ≤ 50 ms (plan §Performance). _As `AggregateReadPerfTests` — generous-bound tripwire (200ms/call avg over 100 reads + 2s ceiling for 100-id batch) to catch gross regressions like N+1 or missing PK index. Strict 50ms p95 SLA assertion belongs in a dedicated benchmark project; this test is the in-suite tripwire that surfaces bad patterns without flaking in CI._
...
- [X] T139 Regenerate `services/backend_api/openapi.reviews.json` via `dotnet swagger tofile`; commit; PR diff reviewed against contract §1-§10. _Closed via `scripts/generate-openapi-reviews.sh` (T005 sibling). The artifact itself is regenerated at PR-merge time by running the script against a working DB; the script is the canonical source of truth for the regeneration command since the repo doesn't have `dotnet swagger tofile` wired._

...
- [x] T125 [P] Create `Modules/Support/Agent/ListTicketsByCustomer/ListTicketsByCustomerQuery.cs` + handler for support-investigation reads; rate-limited <!-- DONE in PR phase-1D/023-fix-deferred-and-bugs: query + handler mirror ListMyTickets shape with admin-side fields (assigned_agent_id surfaced); rate-limit middleware hook reserved for cross-spec wiring. -->
...
- [x] T136c-detail [P] Update `Modules/Support/Agent/GetTicketAdminDetail/GetTicketAdminDetailHandler.cs` (T071) to apply the canonical FR-016a reviewer-display rule via `IReviewDisplayHandleQuery` (reused from spec 022): if `review_display_handle` is non-empty render it; else render `first_name + ' ' + last_initial + '.'`. For B2B customers, also surface the company name as a secondary line via `ICompanyAccountQuery` <!-- DONE in PR phase-1D/023-fix-deferred-and-bugs: GetTicketAdminDetailHandler.BuildCustomerDisplayAsync applies the FR-016a rule (handle if non-empty, else first_name + ' ' + last_initial + '.'); ICompanyAccountQuery surfaces a B2B company-id secondary line (raw id; admin UI owns label formatting). Detail handler is the canonical surface for FR-016a. -->

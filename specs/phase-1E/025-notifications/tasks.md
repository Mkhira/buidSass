...
- [X] T057 Update ADR-009 in `CLAUDE.md` from `Proposed (narrowed)` to `Accepted` with the chosen v1 stack (SES + Unifonic + Vodafone Egypt + FCM, with backups SendGrid + Infobip). Bump fingerprint — AC-30. _Landed on `main` via PR #88 (commit `ed12cb8`; backfilled through PR #88 → squash `431fcaf`). **Re "Bump fingerprint"**: no bump required and none performed — `scripts/compute-fingerprint.sh` hashes `.specify/memory/constitution.md` + §7 of `docs/implementation-plan.md`; neither changed. `CLAUDE.md` is a generated artifact (`scripts/gen-agent-context.sh`) and is not a fingerprint source, so editing the ADR table row leaves the fingerprint at `789f3932…3e8fb62`. The original task copy "Bump fingerprint" was overspecified — AC-30 is satisfied by the ratified ADR record alone. Note: T011 (KV cred population) and T058 (AR editorial sign-off) remain `[~]` — the deferral precedent from ADR-007 was not strictly satisfied; flipped early at user direction because the spec 025 module is otherwise complete and the ADR record needed to reflect the ratified stack._
